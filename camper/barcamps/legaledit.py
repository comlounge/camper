...
class LegalEditForm(BaseForm):
    ...
    imprint             = WYSIWYGField(T("Imprint"), [validators.Required(), validators.Length(min=100, max=20000)], description=T("Please describe in detail who is responsible for this barcamp. This is mandatory for paid barcamps."))
    tos                 = WYSIWYGField(T("Terms of Service"), [validators.Length(min=100, max=20000)], description=T("Please enter your terms of service here."))
    cancel_policy       = WYSIWYGField(T("Cancellation Policy"), [validators.Length(min=100, max=20000)], description=T("Please describe your cancellation policy (make sure it complies to your local law)."))
